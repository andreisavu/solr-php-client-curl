...
class Apache_Solr_Service
{
	...
	/**
	 * Get url contents using curl php extension
	 *
	 * @param	string	$url
	 * @param	string	$data		if not empty the function will do a post request
	 * @param	integer $timeout
	 */	
	protected function _curlGetContents($url, $data=array(), $timeout = false, $contentType = "text/xml; charset=UTF-8") {
		...
		$options = array(
			...
			CURLOPT_USERAGENT      => "php-apache-solr", 
	...
	{
		list($output, $info) = $this->_curlGetContents($url, array(), $timeout);
